...
public:
  ...
  void setAll(string,int,string,int );
  void display();

  // constructor 
  job(string = "Annonymous", int = 18, string = "Unknown", int = 1);
};

job::job(string n, int a, string p, int c){
  name = n;
  age = a;
  prof = p;
  covid = c;
...
void job::setAll(string n,int a,string p, int c){
  name = n;
  age = a;
  prof = p;
  covid = c;
